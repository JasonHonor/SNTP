namespace NetworkTime
{
    ...
    public struct TimeStamp
    {
        ...
        public byte[] ToBytes()
        {
            var byteConverter = new ByteConverter(false);
            var secondsBytes = byteConverter.GetBytes(this.Seconds);
            var fractionBytes = byteConverter.GetBytes(this.Fraction);

            var bytes = new byte[8];
            secondsBytes.CopyTo(bytes, 0);
            fractionBytes.CopyTo(bytes, 0 + 4);
